...
type ManhattanHuddle bool

func (this ManhattanHuddle) move(game *chess.Game) *chess.Move {
	...
	for _, m := range valid {
		...
		eval := ManhattanDist(m.S2(), sq) + (9 - ManhattanDist(m.S1(), sq))
		if eval < ceval {
			ceval = eval
			best = []*chess.Move{m}
		} else if eval == ceval {
...
func (this ManhattanHuddle) name() string {
	return "Manhuddle"
}
